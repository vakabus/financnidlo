...
TEST(ParserTest, PersonParser) {
    ASSERT_EQ(line_parser({"def", "person", "Pepik"}), model::Person("Pepik", {}));
    ASSERT_EQ(line_parser({"def", "person", "a", "b", "c"}), model::Person("a", {"b", "c"}));
    ASSERT_ANY_THROW(line_parser({"def", "person"}));
}

TEST(ParserTest, GroupParser) {
    ASSERT_EQ(line_parser({"def", "group", "Pepik"}), model::Group("Pepik", {}));
    ASSERT_EQ(line_parser({"def", "group", "a", "b", "c"}), model::Group("a", {"b", "c"}));
    ASSERT_ANY_THROW(line_parser({"def", "group"}));
}

TEST(ParserTest, CurrencyParser) {
    ASSERT_EQ(line_parser({"def", "currency", "usd"}), model::Currency("usd"));
